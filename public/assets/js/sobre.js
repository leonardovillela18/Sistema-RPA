...
aboutForm?.addEventListener('submit', async (event) => {
 ...
 try {
  ...
  for(const key of ['introTitle','introText',...Array.from({length:3},(_,i)=>['badge','title','text'].map(k=>k+(i+1))).flat()]) payload.set(key,document.getElementById(key).value.trim());
  for(let i=1;i<=3;i++) { const file=document.getElementById('image'+i).files[0]; if(file) payload.set('image'+i,file); }
  await RPAApi.save('about/save',payload);
...
RPAApi.ready.then(renderAbout).catch(RPAApi.showError);
